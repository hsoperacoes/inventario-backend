...
# Converte datetime salvo no banco (UTC) para horário Brasil (UTC-3)
def hora_brasil_str(dt):
    if dt is None:
        return ""
    # Subtrai 3h para converter UTC -> Brasil
    ...
    return [
        {
            ...
            "data": hora_brasil_str(row.criado_em),
...
def _expandir_bipe_row(b, grupos_map, estoque_por_ean):
    ...
    # ── CORREÇÃO HORÁRIO: converte UTC → Brasil (UTC-3) ──────────────────────
    dt_raw = getattr(b, "atualizado_em", None) or getattr(b, "criado_em", None)
    hora = hora_brasil_str(dt_raw)
...
# ── CORREÇÃO PDF ETIQUETAS: adicionado require_admin corretamente ─────────────
...
def etiquetas_pdf(id_inventario: str = "", request: Request = None, db: Session = Depends(get_db)):
    ...
    if not etiquetas:
        raise HTTPException(status_code=404, detail="Nenhuma etiqueta pendente encontrada.")
    try:
